...
<template>
	<view>
		<view style="display:none">{{getwholepost()}}</view>
		<view style="display:none">{{friend(userInfo._id)}}</view>


			<view v-for="(item,index) in postdata"  :key="index" >
				<view v-for="(itemmm,index) in friend_list"  :key="index" >
						<view v-if="item.user_id==itemmm">
							...
							<view v-for="(item, index) in item.imgList" class="img55">
								...
								<view style="clear: both;"></view>
									<view class="handle-box">
										<view>
						<!--						<span class="label" >
													<view style="display:none">{{checkin(item._id,userInfo._id)}}</view>
													<u-icon @click="changelike(index,1,item._id,userInfo._id,item.thumbs_up_count)" v-if="count[index]==0" name="heart"></u-icon>
													<u-icon @click="changelike(index,0,item._id,userInfo._id,item.thumbs_up_count)" v-if="count[index]!=0" name="heart" style="color: #FF0000;"></u-icon>
												</span>
												<span>		
														<text v-if="count[index]==0"> {{item.thumbs_up_count}}</text>
														<text v-if="count[index]==1"> {{item.thumbs_up_count+1}}</text>
												</span> -->
									...
									-->
										<view class="guli">
											<view @click="enc_others(item._id,item.encourage_content)">
												<span class="label">
	...
	export default {
		...
		methods: {
			...
			async friend(idd){
				...
				let res= await dtbs.collection("friends").where({user_id :idd}).get()
					this.friend_list=res.result.data[0].friend_list
				},
			...
			async getwholepost(){
				const db = uniCloud.database();
				let rese= await db.collection("post").where({exist :"true"}).orderBy('index_post','desc').get()
			...
			enc_others(id,encourage_content){
				uni.navigateTo({
