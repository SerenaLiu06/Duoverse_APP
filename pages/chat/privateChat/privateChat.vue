	...
	<view class="chatInterface">
		<view class="scroll-view">
			...
			<view v-for="(message,index) in messages" :key="message.messageId">
				...
				<view class="message-item" :class="{'self' : message.senderId == (currentUser && currentUser._id)}">
					<view class="avatar"
						  v-if="message.senderId != (currentUser && currentUser._id)">
						<image :src="friend.avatar_file ? friend.avatar_file.url : defaultavatar"></image>
					</view>
					<view class="avatar" v-else>
						<image :src="currentUser.avatar_file ? currentUser.avatar_file.url : defaultavatar"></image>
					</view>
					<view class="content">
		...
		<view class="action-box" v-if="!video.visible">
			<view class="action-top">
				<view :class="[audio.visible ? 'record-icon record-open':'record-icon']"></view>
		...
		onLoad(options) {
			...
			db.collection('uni-id-users').where({
				...
				db.collection('uni-id-users').where({
					...
					this.friend = res2.result.data[0]
					uni.setNavigationBarTitle({
						title : this.friend.nickname
					});
					if(!this.friend.avatar_file){
						this.friend.avatar_file = {
		...
		methods: {
					...
					let textMessage = this.goEasy.im.createTextMessage({
						...
						to : {
							...
							data : {
								...
								avatar:this.friend.avatar_file.url
							}
						},
						notification : {
			...
			loadMoreHistoryMessage() {//历史消息
				...
				this.goEasy.im.history({
					userId: self.friend._id,
					lastTimestamp: lastMessageTimeStamp,
					onSuccess: function (result) {
						//获取本地记录
						let imService = getApp().globalData.imService;
						let localHistory = imService.getPrivateMessages(self.friend._id);
						//添加加载的记录到本地记录尾部
